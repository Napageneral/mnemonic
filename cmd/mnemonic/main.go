...
import (
	...
	"github.com/Napageneral/mnemonic/internal/compute"
	...
	"github.com/Napageneral/mnemonic/internal/identify"
	"github.com/Napageneral/mnemonic/internal/importer"
	"github.com/Napageneral/mnemonic/internal/live"
	"github.com/Napageneral/mnemonic/internal/me"
	"github.com/Napageneral/mnemonic/internal/query"
	"github.com/Napageneral/mnemonic/internal/search"
	"github.com/Napageneral/mnemonic/internal/sync"
	"github.com/Napageneral/mnemonic/internal/tag"
	"github.com/Napageneral/mnemonic/internal/timeline"
...
func main() {
	rootCmd := &cobra.Command{
		Use:   "mnemonic",
		Short: "Unified memory system",
		Long: `Mnemonic aggregates your communications and AI sessions across all channels 
(iMessage, Gmail, Cursor, Codex, etc.) into a unified searchable memory
with identity resolution, semantic search, and analysis.`,
	}
	...
	rootCmd.AddCommand(&cobra.Command{
		Use:   "init",
		Short: "Initialize mnemonic config and database",
		Run: func(cmd *cobra.Command, args []string) {
			type Result struct {
				OK        bool   `json:"ok"`
			...
			result.Message = "Mnemonic initialized successfully"
			...
			if jsonOutput {
				...
				fmt.Println("\nMnemonic initialized successfully!")
			}
		},
	})
	...
	meShowCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if person == nil {
				result := Result{
					OK:      false,
					Message: "Identity not configured. Run 'mnemonic me set --name \"Your Name\"' to configure.",
				}
				if jsonOutput {
					printJSON(result)
				} else {
	...
	adaptersCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(cfg.Adapters) == 0 {
				result.Message = "No adapters configured. Run 'mnemonic connect <adapter>' to configure one."
				if jsonOutput {
					printJSON(result)
				} else {
	...
	connectImessageCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				fmt.Println("\nRun 'mnemonic sync' to sync iMessage events")
			}
		},
	}
	...
	connectGmailCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				fmt.Printf("\nRun 'mnemonic sync --adapter %s' to sync Gmail events\n", adapterName)
			}
		},
	}
	...
	connectCalendarCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				fmt.Println("  brew install steipete/tap/gogcli")
				fmt.Printf("  gog auth add %s\n", account)
				fmt.Printf("\nRun 'mnemonic sync --adapter %s' to sync calendar events\n", adapterName)
			}
		},
	}
	...
	connectContactsCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				fmt.Println("\nRun 'mnemonic sync --adapter " + adapterName + " --full' to ingest contacts and unify identities")
			}
		},
	}
	...
	connectGoogleCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if startBackfill {
				...
				fmt.Println("Run: mnemonic sync status")
			} else {
	...
	connectCursorCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				fmt.Println("\nRun 'mnemonic sync --adapter cursor' to sync Cursor AI sessions")
			}
		},
	}
	...
	connectXCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				fmt.Println("\nRun 'mnemonic sync' to sync X events (bookmarks, likes, mentions)")
			}
		},
	}
...
mnemonic sync
mnemonic sync imessage
mnemonic sync --adapter gmail --full
mnemonic sync --background
`),
		Run: func(cmd *cobra.Command, args []string) {
			type Result struct {
				OK       bool                 `json:"ok"`
			...
			if background {
				...
				logPath := filepath.Join(dataDir, "mnemonic-sync.log")
	...
	syncCmd.Flags().Bool("background", false, "Run sync in background (writes logs to mnemonic-sync.log)")
	...
	syncStatusCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(jobs) == 0 {
				result := Result{OK: true, Message: "No job status found yet. Run: mnemonic sync"}
				if jsonOutput {
					printJSON(result)
				} else {
					fmt.Println(result.Message)
	...
	importCmd := &cobra.Command{
		Use:   "import",
		Short: "Import external data into mnemonic",
	}
	...
	importMBoxCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				fmt.Printf("  mnemonic sync --adapter %s\n", adapterName)
			}
		},
	}
	...
	busCmd := &cobra.Command{
		Use:   "bus",
		Short: "Inspect the mnemonic event bus",
	}
	...
	identifySuggestCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				fmt.Println("Use 'mnemonic identify suggestions' to review them")
			}
		},
	}
	...
	identifySuggestionsCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				if len(infos) == 0 {
					fmt.Println("No pending suggestions. Run 'mnemonic identify suggest' to generate some.")
				} else {
					...
					fmt.Println("Use 'mnemonic identify accept <id>' or 'mnemonic identify reject <id>'")
				}
			}
		},
	}
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				if resolveAutoMerge {
					...
					fmt.Println("\nUse 'mnemonic identify merges' to review pending suggestions")
				}
			}
		},
	}
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				if len(infos) == 0 {
					...
					fmt.Println("\nRun 'mnemonic identify resolve' to generate suggestions")
				} else {
...
func checkAdapterStatus(name string, adapter config.AdapterConfig) string {
	...
	case "aix", "aix-events", "aix-agents":
		home, err := os.UserHomeDir()
		if err != nil {
			return "error"
		}
